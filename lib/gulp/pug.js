...
const compilePugEventPages = () => {
  ...
  // Feature this calendar year, last calendar year, and all upcoming events on the /events page.
  const lastYear = new Date().getFullYear() - 1
  const featuredEvents = Object.keys(eventsByYear)
    .map((year) => parseInt(year, 10))
    .sort((a, b) => b - a)
    .filter((year) => year >= lastYear)
    .flatMap((year) => eventsByYear[year])
